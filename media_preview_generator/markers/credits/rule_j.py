...
# (:func:`rows_before`), roll or story, no further back than 30 s before the earliest credits start the decision keeps
# (``detector.find_credits``, ``decide.earliest_credits_start_ms``).
...
def text_all_through(rows: Sequence[Row], coarse: Coarse) -> bool:
    ...
    """
    if too_little_story(rows, coarse):
        return True
...
def too_little_story(rows: Sequence[Row], coarse: Coarse) -> bool:
    """Whether the run starts less than ``STORY_BEFORE_RUN_S`` after the first row: too little read before it to tell
    a roll from text on screen all through (:func:`text_all_through`'s first test). While it holds after the first step
    before the tail, the detector reads another (``detector.find_credits``)."""
...
def opens_on_the_run(rows: Sequence[Row], coarse: Coarse, params: RuleParams = RULE_J) -> bool:
    ...
    story, is one such run). Only the tail is asked: once the run has carried on into the step before it, the detector
    goes on while :func:`too_little_story` holds, lit rows before the run or not.
    ...
    Raises:
        ValueError: ``rows`` holds no run, so there is none to ask about. This step finds the runs again, so it is
            ...
            two cards in it).
    """
    if not too_little_story(rows, coarse):
        return False
    runs = credit_runs(rows, params)
    if not runs:
...
def joined_before(
    ...
    is what the run's start needs before it.
...
def rows_before(before: Sequence[Row], rows: Sequence[Row]) -> list[Row]:
    """``before`` put ahead of ``rows``, without its rows at or after ``rows``' first (those are ``rows``' own), each
    window in its own decode order.

    Args:
        before: Keyframe rows of the window read before ``rows``.
        rows: The rows read so far.

    Returns:
        The joined rows.
    """
    first_s = min(row[0] for row in rows)
    return [*(row for row in before if row[0] < first_s), *rows]
