    ...
    *,
    ...
    Args:
        ...
        settings_provider: The saved settings right now; Plex checks them before every database write, so a job
            started earlier can't write after Intro & Credits was turned off. Jellyfin needs no such guard: its
            writes go through the server's own API and the pipeline checks the saved settings first.
