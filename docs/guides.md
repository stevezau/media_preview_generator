...
back two minutes at a time until it finds where they start — never past the middle of the file, and for no longer
than the ten minutes a single decode is allowed. A start found that way still has to be in the last quarter of the file (and for a movie
within 15 minutes of the end, or your movie window if that is longer), so a window that covers your credits is still
the surer choice. A longer window takes longer to decode for every file, and a window you choose also lets credits start that early: without it a start earlier than the last quarter of the file, or (for a movie window above 15 minutes) more than 15 minutes from the end, would be found and then refused. It never lets credits start before the middle of the file. Changing it reads files
