...
def analyse(files, window, max_len=120):
    ...
    def get(f):
        if window == "intro": return cached_fp(f, 0, min(900, durs[f]*0.35 if durs[f] >= 300 else durs[f]))
        s = max(0.0, durs[f]-fp.CREDITS_WINDOW_S); return cached_fp(f, s, durs[f]-s)
    with cf.ThreadPoolExecutor(2) as ex: fps = dict(zip(files, ex.map(get, files)))
