...
| **`decide-rules/`** (2026-09-25) | The five decision rules proposed after the production audit (SkipDB alone, credit text against a credits chapter, online credits ending past the file, blocking by a source that can't decide alone, another release's intro beside season audio): every set at decide level before/after, the production replay, frame sheets | `README.md` (rules and outcomes), `measure_final.sh`, `variants.py`, `prod_replay.py`, `intro_sets.py`, `credits_sets.py`, `online_set.py`; local-only `local/` (the audit's `post.db`, results, sheets) |
...
| **`end-picture-scaler/`** | Season audio v8: the end-picture check on the credit text decode's one scaler; the five intro sets on NVIDIA and the CPU, before and after; the production re-run cost | `README.md`, `ep_sets.py`, `compare_runs.py`, `frames_vendor_check.py`, `ep_prod_cost.py`, `run_all.sh` |
...
| **`replaced-speed-endcard/`** | Three intro fixes after #312 (§14 2026-09-25): season audio hears which speed a file's audio plays at (RuPaul's Drag Race UK S08E04), the carry-over of §5.5 rule 14 (Tomb Raider King S01E12, RPDR), the end card in the end-picture check (Tomb Raider King S01E12); the sets, Bones and production replays | **`README.md`**, `speed_scores.py`, `bones_speed.py`, `carry_prod.py`, `trk_frames.py`, `trk_sheet.sh`, `ep_variants_sets.py`, `card_all_failing.py`, `prod_card.py` |
...
| `credits/` (2026-09-24/25) | Credits text versions 4 and 5 | **`small-text-retry.md`** (the write-up; **"Before release"**: the plex-host checks still owed; **the Accused epilogue-card gap**; version 5: **"The audit's two regressions"**), `small_text_retry.py`, `small-text-retry/` (`force640.py`, `stretch_probe.py`), **`isurvived/`** (I Survived a Serial Killer S01: the regression set's frame-checked truth, sheets and traces, `trace.py`), **`vendor-scaler/`** (one scaler on every vendor; `mpgverify/`), `credit-text-alone/` (a measured proposal, not taken) |
