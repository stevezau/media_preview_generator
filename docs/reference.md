...
| POST | `/api/workers/add` · `/api/workers/remove` | Add or remove pool workers live (`{"worker_type": "CPU"\|"GPU", "count": N}`). A CPU change is also saved as the **CPU workers** setting, just like the dashboard's +/- buttons. That setting goes from 0 to 32, and an add that would pass 32 gets a `400` with nothing changed. A busy worker that's removed finishes its current file first. GPU changes aren't saved; they last until the next Settings save, job start or restart. |
