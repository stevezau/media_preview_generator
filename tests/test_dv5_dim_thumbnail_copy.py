"""Without hardware Vulkan, Dolby Vision Profile 5 thumbnails come out dim.

The DV5 path no longer runs libplacebo on a software rasteriser (which
painted a green overlay); it extracts frames with no tone mapping, so the
thumbnails are dim.  Every user-facing surface that describes the no-Vulkan
state must say so, and must not promise or blame a green overlay.  Each
surface is its own cell — they are independent copies that drifted together.
The behaviour itself is pinned in ``test_media_processing.py``
...
def _assert_describes_dim_thumbnails(text: str) -> None:
    lowered = text.lower()
    assert "green" not in lowered, f"still describes a green overlay: {text!r}"
    assert "dim" in lowered, f"doesn't say the thumbnails come out dim: {text!r}"
...
class TestProbeLogs:
    def test_launch_failure_warning_says_dim_when_ffmpeg_missing(self) -> None:
        ...
        _assert_describes_dim_thumbnails(messages[0])

    def test_unexpected_error_warning_says_dim_when_probe_raises(self) -> None:
        from media_preview_generator.gpu.vulkan_probe import _run_vulkan_probe
    ...
    def test_debug_line_says_dim_when_ffmpeg_lacks_vulkan(self) -> None:
        from media_preview_generator.gpu.vulkan_probe import _run_vulkan_probe
        ...
        _assert_describes_dim_thumbnails(hits[0])
    ...
    def test_first_probe_outcome_says_dim_and_names_the_fix(self, device: str | None, level: str) -> None:
...
class TestDashboardWarning:
    ...
    def test_warning_says_dim_when_vulkan_is_software(self, gpus, render_nodes, diag) -> None:
        ...
        _assert_describes_dim_thumbnails(warning)
        assert "software rendering, which" not in warning

    def test_loader_rejected_log_says_dim_when_nvidia_checks_pass(self) -> None:
        from media_preview_generator.web.routes.api_vulkan import _get_vulkan_info
    ...
    def test_notification_title_says_dim_when_vulkan_is_software(self) -> None:
        ...
        _assert_describes_dim_thumbnails(notification["title"])

    def test_debug_bundle_intro_never_mentions_green(self) -> None:
        ...
        assert status == 200
        assert "green" not in body.lower()
